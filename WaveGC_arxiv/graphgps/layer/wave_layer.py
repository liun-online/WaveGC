...
from graphgps.layer.revgnn import RevGNN
...
class Wave_Conv(nn.Module):
    ...
    def forward(self, x, batch):
        ...
        for t in range(self.t_number):
            filters = evc_dense @ (diag_filter_signals_after[:, t, :].unsqueeze(-1) * evc_dense_t)
...
class WaveLayer(nn.Module):
    ...
    def __init__(self, layer_num, layer_idx,dim_h,
        ...
        elif local_gnn_type == 'Rev':
            self.local_model = RevGNN(dim_h, dim_h, dim_h, 1, 0.5, 2, norm = 'LN')
