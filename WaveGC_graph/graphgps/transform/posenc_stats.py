...
def compute_posenc_stats(data, pe_types, is_undirected, cfg):
    ...
    path_eva = eig_path+"eva_"+str(data_idx)+".pt"
    path_evc = eig_path+"evc_"+str(data_idx)+".pt"
    if os.path.exists(path_eva) and os.path.exists(path_evc):
        evals = torch.load(path_eva)[: N].numpy()
        evects = torch.load(path_evc)[:N, :N].numpy()
    else:
