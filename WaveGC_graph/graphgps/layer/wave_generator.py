...
class Wave_generator(nn.Module):
    ...
    def forward(self, batch):    
        ...
        coe_scaling = F.softmax(coe_scaling.sum(1) / (batch.length.view(-1,1)+1e-8), dim=-1)  # B*num_n
        
        coe_wavelet = self.decoder_wavelet(eig_filter)  # B*N*num_n
        coe_wavelet[eig_mask] = 0.
        coe_wavelet = F.softmax(coe_wavelet.sum(1) / (batch.length.view(-1,1)+1e-8), dim=-1)  # B*num_n
