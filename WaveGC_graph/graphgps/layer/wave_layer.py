...
class Wave_Conv(nn.Module):
    def __init__(self, dim_h, act, dropout, num_J, layer_idx=-1):
        super().__init__()
        self.dim_h = dim_h
        assert self.dim_h % (num_J+1) == 0
        self.single_out = self.dim_h//(num_J+1)
        self.activation = register.act_dict[act]()
        self.drop = nn.Dropout(dropout)
        self.t_number = num_J + 1
        self.wave_lin = nn.ModuleList([nn.Linear(dim_h, self.single_out) for _ in range(num_J+1)])
        self.fusion = nn.Linear(dim_h, dim_h)
        self.layer_idx = layer_idx

    def forward(self, x, batch):
        comb = []
        evc_dense = batch.evc_dense # B N N
        evc_dense_t = batch.evc_dense.transpose(1, 2) # B N N
        ...
        curr_signals = rearrange(curr_signals, 'b n j-> j b n') # (J+1) B N 

        basis = evc_dense @ (curr_signals.unsqueeze(-1) * evc_dense_t) # (J+1) B N N
        if cfg.WaveGC.normalize:
            basis = F.normalize(basis, dim=-1)
        x, mask = to_dense_batch(batch.x, batch.batch) # B N d
        ...
        for t in range(self.t_number):
            curr_x = self.drop(self.activation(self.wave_lin[t](x[t])))
            curr_x[~mask] = 0.
            proj_x.append(curr_x)
        proj_x = torch.stack(proj_x, 0) # (J+1) B N d'

        #### 2nd basis ####
        x = basis @ proj_x
        comb = torch.cat([i for i in x], -1)
...
class WaveLayer(nn.Module):
    ...
    def __init__(self, layer_num, layer_idx,dim_h,
                 layer_type, num_heads, num_J, act='relu',
            ...
            if cfg.WaveGC.trans_use:
                if layer_idx!=0 and layer_idx!=layer_num-1:
                    if cfg.WaveGC.weight_share:
                        ...
                        self.self_attn = Wave_Conv(dim_h, act, wave_drop, num_J,layer_idx-1)
                ...
                if cfg.WaveGC.weight_share:
                    ...
                    self.self_attn = Wave_Conv(dim_h, act, wave_drop, num_J,layer_idx)
                self.attn_flag = 'wavelet'
        else:
