...
class Wave_generator(nn.Module):
    ...
    def forward(self, batch):    
        ...
        coe_scaling = self.sigmoid(coe_scaling.sum(1) / (batch.length.view(-1,1)+1e-8))  # 1*num_n
        ...
        coe_wavelet = self.sigmoid(coe_wavelet.sum(1) / (batch.length.view(-1,1)+1e-8))  # 1*num_n
