...
from torch_geometric.datasets import (CoraFull,Amazon, Actor, GNNBenchmarkDataset, Planetoid,
                                      TUDataset, WebKB, WikipediaNetwork, ZINC, Coauthor, HeterophilousGraphDataset)
...
def load_dataset_master(format, name, dataset_dir):
    ...
    if format.startswith('PyG-'):
        ...
        if pyg_dataset_id == 'Actor':
            ...
            if name == "cs":
                pre_transform_in_memory(dataset, partial(generate_splits, g_split = cfg.dataset.split[0]))
                get_eigns(dataset, './datasets/Coauthor/CS/')
    ...
    # idx_train = torch.arange(len(data.train_mask))[data.train_mask.bool()]
    # idx_val = torch.arange(len(data.train_mask))[data.val_mask.bool()]
    # idx_test = torch.arange(len(data.train_mask))[data.test_mask.bool()]
    # print(idx_train)
    # torch.save(idx_train,'idx_train.pt')
    # print(idx_val)
    # torch.save(idx_val,'idx_val.pt')
    # print(idx_test)
    # torch.save(idx_test,'idx_test.pt')
...
def get_eigns(dataset, path):
    if osp.exists(path+"/evals.npy") and osp.exists(path+"/evects.npy"):
        pass
    else:
        print('Generating eigenvalues and eigenvectors...')
        data = dataset.get(0)
        N=data.num_nodes
        ...
        np.save(path+"/evects.npy", evects)
        print('Finish!')